...
int32 UInventoryComponent::ConsumeItem(class UItem* Item, const int32 Quantity)
{
	if (GetOwner() && GetOwner()->HasAuthority() && Item)
	{
		...
		//We now have zero if this item, remove it from the inventory
...
FItemAddResult UInventoryComponent::TryAddItem_Internal(class UItem* Item)
{
	if (GetOwner() && GetOwner()->HasAuthority())
	{
		...
		if (Item->bStackable)
		{
			//Somehow the item's quantity went over the max stack size. This shouldn't ever happen
			...
			if (UItem* ExistingItem = FindItem(Item))
			{
				if (ExistingItem->GetQuantity() < ExistingItem->MaxStackSize)
				{
					...
					if (!FMath::IsNearlyZero(Item->Weight))
					{
						...
						{
							ErrorText = FText::Format(LOCTEXT("InventoryTooMuchWeightText", "Couldn't add the entire stack of {ItemName} to Inventory."), Item->ItemDisplayName);
						}
					}
					else if (ActualAddAmount < AddAmount)
					{
						//If the item weighs none and we can't take it, then there was a capacity issue
				...
				{
					return FItemAddResult::AddedNone(AddAmount, FText::Format(LOCTEXT("InventoryFullStackText", "Couldn't add {ItemName}. You already have a full sstack of this item."), Item->ItemDisplayName));
				}
			}	
			else
			{
